...
strandfile="/afm01/UQ/Q3490/SSC/QCd/Human1Mv1_C-b36.Ilmn.strand"
strandbuild37="/afm01/UQ/Q3490/SSC/QCd/Human1Mv1_C-b37.strand"
ref="/afm01/UQ/Q3490/SSC/QCd/human_g1k_v37.fasta"
bfile="SubjectGenotypeFile.AGP.phs000267.v1.p1.20120806"

##rcc/flashlite version
...
strandfile="/QRISdata/Q3490/SSC/QCd/Human1Mv1_C-b36.Ilmn.strand"
strandbuild37="/QRISdata/Q3490/SSC/QCd/Human1Mv1_C-b37.strand"
ref="/QRISdata/Q3490/SSC/QCd/human_g1k_v37.fasta"
bfile="SubjectGenotypeFile.AGP.phs000267.v1.p1.20120806"


###2.1 generate QCd file~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~####
system(paste0("plink --bfile ", genodir, bfile, " --chr 1-23, 25 --geno 0.05 --maf 0.01 --mind 0.2 --hwe 1e-6 --make-bed --out ", OUTDIR, bfile, "_allchrs"))
...
###2.2 update SNP IDs ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~####
##no need to update as most SNP IDs have been coded as rs.. only 4 is the same with strand file as SNP***.
##but the code for indels should be updated/removed, here we removed with only ~160
pos <- read.table(paste0(strandfile), stringsAsFactors = F)
bim3 <- read.table(paste0(OUTDIR, bfile, "_allchrs_update.bim"), stringsAsFactors = F)
als <- c("A", "G", "C", "T")
...
system(paste0("/QRISdata/Q3490/SSC/QCd/update_build.sh ", OUTDIR, bfile, "_allchrs_update  ", strandbuild37, " ",  OUTDIR, bfile, "_allchrs_flip"))
...
# write.table(sorted.a, file = paste0(OUTDIR, bimname2, ".bim"), col.names = F, row.names = F, quote = F, sep = "\t")
# 
# ## rename .fam .bed as .bim
# system(paste0("mv ", OUTDIR, bimname2, ".fam ", OUTDIR, bimname2, ".fam"))
# system(paste0("mv ", OUTDIR, bimname2, ".bed ", OUTDIR, bimname2, ".bed"))
...
###2.5 Genotype QCs ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~####
system(paste0("plink --bfile ", OUTDIR, bfile, "_allchrs_flip2 --autosome --make-bed --out ", OUTDIR, bfile, "_qcd_autosome"))
system(paste0("plink --bfile ", OUTDIR, bfile, "_allchrs_flip2 --chr 23, 25 --make-bed --out ", OUTDIR, bfile, "_qcd_chrX"))
...
######autosome only
system(paste0("plink --bfile ",  OUTDIR, bfile, "_allchrs_flip2 --autosome --recode vcf --out ", OUTDIR, bfile, "_allchrs_flip2_recode"))
system(paste0("vcf-sort ", OUTDIR, bfile, "_allchrs_flip2_recode.vcf | bgzip -c > ", OUTDIR, bfile, "_allchrs_flip2_recode.vcf.gz"))


## remove the SNPs in hh file for X chromosome if a hh file was generated. otherwise the imputation will fail.
system(paste0("plink --bfile ",  OUTDIR, bfile, "_allchrs_flip2 --chr 23  --set-hh-missing --recode vcf --out ", OUTDIR, bfile, "_allchrs_flip2_recode_chrX"))
system(paste0("sed 's/23/X/' ", OUTDIR, bfile, "_allchrs_flip2_recode_chrX.vcf > ", OUTDIR, bfile, "_allchrs_flip2_recode_chrXII.vcf"))
system(paste0("vcf-sort ", OUTDIR, bfile, "_allchrs_flip2_recode_chrXII.vcf | bgzip -c > ", OUTDIR, bfile, "_allchrs_flip2_recode_chrX.vcf.gz"))
